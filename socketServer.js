...
io.on("connection", (socket) => {
  ...
  socket.on("asianOdds", async (data) => {
    let result = await asianOdd(data);
    io.to(socket.id).emit("asianOdd_response", result);
    intervals[socket.id] = setInterval(async () => {
      result = await asianOdd(data);
      io.to(socket.id).emit("asianOdd_response", result);
    }, 1000);
  });
