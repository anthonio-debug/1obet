...
async function getFinalReport(req, res) {
  ...
  if (currentUser.createdBy !== 0) {
    ...
    if (parentUserData) {
      results.negativeClients.push({ userName: parentUserData.userName, userId: parentUserData.userId, clientPL: (currentUser.credit - currentUser.cash) });
      results.totalNegativeClientPL = results.totalNegativeClientPL + (currentUser.credit - currentUser.cash);
    } else {
      results.negativeClients.push({ userName: currentUser.userName, userId: currentUser.userId, clientPL: (currentUser.credit - currentUser.cash) });
      results.totalNegativeClientPL = results.totalNegativeClientPL + (currentUser.credit - currentUser.cash);
    }
  } else {
    results.negativeClients.push({ userName: currentUser.userName, userId: currentUser.userId, clientPL: (currentUser.credit - currentUser.cash) });
