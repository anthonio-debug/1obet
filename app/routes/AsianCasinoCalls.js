...
const CasinoDebits        = require('../models/AsiancasinoCalls');
    ...
    await session.withTransaction(async () => {
      ...
      let userResponse = await users.updateOne(
        ...
        for (const user of parentUser) {
          ...
          let cash = {
            userId: user.userId,
            description: "Casino Batting Share",
            createdBy: 0,
  ...
  try {
    ...
    await session.withTransaction(async () => {
      ...
      if (payload.amount > 0) {
        ...
        let cash = {
          userId: user.userId,
          description: "Casino Batting winning",
          createdBy: 0,
...
router.get('/asiancasino', asiancasino);
