  ...
  try {
    ...
    User.findOne(query, (err, user) => {
      ...
      Cash.aggregate(cashPipeline, async (err, result) => {
        ...
        // Fetch bet info for each result
        for (let i = 0; i < results.length; i++) {
          if (results[i].betId) {
            try {
              const betInfo = await Bet.findOne({ _id: results[i].betId });
              results[i] = { ...results[i], ...betInfo.toObject() }; // Merge bet info into the result
            } catch (err) {
              continue; // Continue if there's an error fetching bet info
            }
          }
        }
        ...
        const responseData = {
          ...
          results: {
            ...
            limit: limit ? limit : 0,
            page: page ? page : 0,
            pages: limit && result[0].metadata[0].total
