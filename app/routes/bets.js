  ...
  try {
    ...
    if (asianOdd) {
      ...
      if (!eventDetail.betAllowed) {
        return res
          .status(404)
          .send({message: "Batting Not Allowd on this Match1",eventDetail.betAllowed});
      }
      if (eventDetail.status.toUpperCase() != "OPEN") {
        return res
          .status(404)
          .send({message: "Batting Not Allowd on this Match2",eventDetail.status.toUpperCase()});
      }
      if (eventDetail.matchStopStatus) {
        return res
          .status(404)
          .send({message: "Batting Not Allowd on this Match3",eventDetail.matchStopStatus});
      }
