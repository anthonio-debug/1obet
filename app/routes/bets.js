...
const updateParentUserBalance = async (parentUsersIds, winningAmount, matchId = 0 ) => {
  ...
  parentUsersData.forEach(user => {
    const dummy = {...user}
    let current = parseInt(dummy.downLineShare);
    dummy['commission'] = current - prev;
    prev = current;
    parentUsers.push(dummy);
    console.log("user ====== ", dummy);
    console.log("commission ====== ", dummy.commission);
  });
...
async function placeBet(req, res) {
  ...
  try {
    ...
    if (marketId == '7' || marketId == '4339'){
      ...
      if (!subMarketDetail) {
        return res.status(404).send({ message: 'subMarketDetails not found' });
      }
    }else {
