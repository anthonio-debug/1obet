...
async function cronOdds2(req, res) {
  ...
  try {
    // Get data from the bookmaker API
    const bookmakerResponse = await axios.get(bookmakerUrl);
    const marketsData = bookmakerResponse.data;

    // Array to store the market IDs of relevant markets (Match Odds, To Win the Toss, Bookmaker)
    const sendMarketIds = [];
    ...
    if (marketsData && marketsData.length > 0) {
      for (const element of marketsData) {
        ...
        // Save only "Match Odds", "To Win the Toss", and "Bookmaker" markets
        if (
          element.marketName === 'Match Odds' || 
          element.marketName === 'To Win the Toss' || 
          element.marketName === 'Bookmaker'
        ) {
          ...
          // If the market is not found, save it as a new entry
          if (!marketID) {
          ...
          // If the market is found, update its totalMatched value
          else {
            console.log("Updating market id collection for existing market");
    ...
    // Get odds for the markets with relevant market IDs
    let result = [];
    console.log("Sending market ids:", sendMarketIds);
