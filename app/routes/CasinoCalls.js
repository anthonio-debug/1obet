...
async function pokerexposure(req, res) {
  ...
  try {
    ...
    if (existingCall) {
      ...
      if ((user.availableBalance + Math.abs(existingCall.calculateExposure)) < (requestData.calculateExposure * auracasinoMultiples)) {
        responseData = {
          errorCode: 1,
      ...
      if ((user.availableBalance) < Math.abs(requestData.calculateExposure * auracasinoMultiples)) {
        responseData = {
          errorCode: 1,
          errorDescription: 'Insufficient Balance',
        };
    ...
    while (retries < maxRetries) {
      ...
      try {
        ...
        await ParentsExpControl(user, requestData, [], 1, session, 0)
        responseData = {
          "status": 0,
        ...
        if (session.inTransaction()) {
          await session.abortTransaction();  // Ensure session is aborted on error
        }
        session.endSession();
      }
    }//end while loop
...
async function pokererresults(req, res) {
  ...
  while (retries < maxRetries) {
    try {
      ...
      await ParentsExpControl(user, requestData, existingCall, 2, session, UsercommissionAmount);
      await session.commitTransaction();
      break; // Exit loop if transaction succeeds
    } catch (error) {
      if (retries < maxRetries) {
...
async function ParentsExpControl(userToUpdate, requestData, existingCall, action, session, UsercommissionAmount) {
  ...
  if (action == 2) {
    ...
    profitLoss = profitLoss - UsercommissionAmount
    let downpl = requestData[0].downpl * auracasinoMultiples;
...
async function getAllCasinoCallsByCreateAt(req, res) {
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
...
async function getAllCasinoCalls(req, res) {
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
  console.log("@@@@@@@@@@@@@@@@@@@@@@@@");
...
router.get('/poker/getAllCasinoCalls/:userId', getAllCasinoCalls);
