  ...
  try {
    ...
    */
    ...
    if (action === 0) {
      ...
      if (difference < 0) {
        // User lost case: negative difference
        console.log("User lost. Handling loss scenario...");

        const bettorLostAmount = Number(((debit - credit) * casinoMultiples).toFixed(3));
        ...
        allTransactions.push(logData);

        // Distribute losses among shareholders
        const parentUserIds = [];
        let currentUserId = user.userId;

        while (currentUserId) {
            const parentUser = await users.findOne({ userId: currentUserId }, { session });
            if (parentUser.role === "0") break; // Stop if admin is reached
        ...
        let downMovingAmount = bettorLostAmount;
        let prevCommission = 0;

        for (const parent of parentUsers) {
            let currentCommission = parent.downLineShare;
            parent["commission"] = currentCommission - prevCommission;
            prevCommission = currentCommission;
        }

        for (const parent of parentUsers) {
            const commissionPercentage = parent.commission / 100;
            const availableBalance = Number((parent.availableBalance + commissionPercentage * bettorLostAmount).toFixed(3));
            const balance = Number((parent.balance + commissionPercentage * bettorLostAmount).toFixed(3));
            const clientPL = parent.clientPL - (100 - parent.downLineShare !== 100) 
                ? Number((parent.clientPL - ((100 - parent.downLineShare) / 100) * bettorLostAmount).toFixed(3)) 
                : 0;

            await users.updateOne(
                { _id: parent?._id },
                { $set: { availableBalance, clientPL, balance } },
                { session }
            );
            ...
            let parentTransaction = {
                ...
                marketId: payload.game_id,
                matchId: payload.game_id,
                downLineAmount: downMovingAmount
            };

            allTransactions.push(parentTransaction);
            downMovingAmount = Number((downMovingAmount - commissionPercentage * bettorLostAmount).toFixed(3));
            commissionFrom = parent.userId;
        }

        await Cash.insertMany(allTransactions);

    } else if (difference > 0) {
        // User won case: positive difference
        console.log("User won. Handling win scenario...");

        const bettorWonAmount = Number(((debit - credit) * casinoMultiples).toFixed(3));

        // Update user's balance and clientPL
        const updatedAvailableBalance = user.availableBalance + bettorWonAmount;
        const updatedClientPL = Number((user.clientPL + (difference * casinoMultiples)).toFixed(3));
        const updatedBalance = Number((user.balance + bettorWonAmount).toFixed(3));
        ...
        // Distribute winnings across shareholders
        const parentUserIds = [];
        let currentUserId = user.userId;
        ...
        let commissionFrom = user.userId;
        let downMovingAmount = bettorWonAmount;
        let prevCommission = 0;
        ...
        for (const parent of parentUsers) {
            const commissionPercentage = parent.commission / 100;
            const availableBalance = Number((parent.availableBalance - commissionPercentage * bettorWonAmount).toFixed(3));
            const balance = Number((parent.balance - commissionPercentage * bettorWonAmount).toFixed(3));
            const clientPL = parent.clientPL + (100 - parent.downLineShare !== 100) 
                ? Number((parent.clientPL + ((100 - parent.downLineShare) / 100) * bettorWonAmount).toFixed(3)) 
                : 0;
            ...
            allTransactions.push(parentTransaction);
            downMovingAmount = Number((downMovingAmount - commissionPercentage * bettorWonAmount).toFixed(3));
            commissionFrom = parent.userId;
        }
