  ...
  try {
    ...
    for (const tran of groupedTransactions) {
      ...
      roundIds.forEach(rounds => {
        console.log("rouuuuuuuuuuuuuuuuuuuundID",round._id)
    ...
    */
    ...
    if (action === 0) {
     ...
     const updateUser=   await users.updateOne(
        ...
        //       calledArea: " difference < looks bal 0 ",
        //       createdBy: 0,
        ...
        //       credit: lastMaxWithdraw?.credit || 0,
        //       creditRemaining: lastMaxWithdraw?.creditRemaining || 0,
        //       calledArea: " difference < loos bal 8else 0 ",
        //       createdBy: 0,
        //       casinoBetAmount: debit,
        //       event: GameName,
        //       betDateTime: betTime,
        ...
        let UserWinBetTrans = {
          ...
          calledArea: " difference < 34 win 0 ",
          cashOrCredit: "Bet",
          cash: lastMaxWithdraw ? lastMaxWithdraw.cash : 0,
          credit: lastMaxWithdraw?.credit || 0,
          creditRemaining: lastMaxWithdraw?.creditRemaining || 0,
        ...
        for (const user of parentUser) {
          ...
          let betTransaction = {
            ...
            credit: lastMaxWithdraw?.credit || 0,
            creditRemaining: lastMaxWithdraw?.creditRemaining || 0,
            calledArea: " difference < 3 8win 0 ",
            cashOrCredit: "Bet",
            sportsId: "6",
            event: GameName,
            marketId: payload.game_id,
          ...
          let commissionTransaction = {
            ...
            calledArea: " difference < 4 9win 0 ",
