...
async function updateOddsFormLimitless() {
  ...
  try {
    // ,
    const marketsData = await MarketIDs.find({ marketName: "Match Odds", ReadyForOdds: true, status:"OPEN" });
    ...
    console.log("===+++++++++++++++++++++ marketsData= ",marketsData);
          ...
          if (marketsData && marketsData.length > 0) {
              ...
              for (let element of marketsData) {
                ...
                const marketStartTime = new Date(element.openDate).getTime();
    ...
    try {
      for (const id of sportsIds) {
        ...
        if (documents && documents.Id) {
          ...
          await inPlayEvents.updateMany({ Id: documents.Id }, { $set: { lastCheckMarket: Date.now() } });
  ...
  async function handleSetInplay() {
    ...
    if (documents.length > 0) {
      for (const document of documents) {
        await inPlayEvents.updateOne({ Id: document.Id }, { $set: { inplay: true } });
      }
    }
  }
}
