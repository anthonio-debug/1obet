...
console.log("userPrevClientPL updated..........................................: : ",userPrevClientPL+diff);
  ...
  await Deposits.create({
    ...
    /// Follownig are assignments for commissions, downlines, uplines etc to parents...
    //await updateParentBalances(userToUpdate.userId,diff,bet);

    async function updateParentBalances(userToUpdateuserId,diff,bet) { 
      const parentUserIds = await getParents(userToUpdateuserId);
      const parentUser = await User.find({
        userId: { $in: parentUserIds },
        isDeleted: false
      }).sort({ userId: -1 });
      if (!parentUser) {
        ...
        let commissionFrom = userToUpdateuserId;
        for (const user of parentUser) {
    ...
    }//updateparentbalances function closed
