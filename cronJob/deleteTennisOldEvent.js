...
const deleteTennisOldEvent = () => {
  ...
  cron.schedule('*/5 * * * *', async () => {
    try {
      // Suppose 
      const tenis     = moment(new Date(Date.now() - 110 * 60 * 1000)).format("MM/DD/YYYY h:mm:ss A Z");
      const eventIds  = await Events.distinct('Id',{ sportsId: '2', openDate : {$lt: tenis}})
